import threading, random, zipfile, csv
...
def change_region(email, password, proxy, country_code, country):
    ...
    print(fg.light_red + f"[ Nike ] [ {time.strftime('%H:%M:%S', time.localtime())} ] [ {email} ]" + fg.dark_grey + " • " + fg.green + "Successfully changed country of account!")
    driver.quit()
    ...
    embed = DiscordEmbed(title=f'Successfully changed account country', description='', color='bc252c')
    ...
    embed.add_embed_field(name='Country', value=f':flag_{country.lower()}:', inline=True)
...
with open('Nike/region_changer.csv', newline='') as input_file:
    csv_file = csv.reader(input_file)
    try:
        for row in csv_file:
        
            if row != ["email", "password", "proxy", "countrycode", "new region"]:
                task += 1
                thread = threading.Thread(target=change_region, args=(row[0], row[1], row[2], row[3], row[4], ))
...
embed = DiscordEmbed(title=f'KenkiTools Nike Logger', description=f'{key_username} changing account country', color="bc252c")
embed.set_footer(text=f"KenkiTools", icon_url="https://media.discordapp.net/attachments/878354931201998859/1059895159959076975/kenki_logo_render.png")
embed.set_timestamp()
