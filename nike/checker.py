import random, csv, threading, zipfile
...
def check_account(email, password, proxy, country_code):
    ...
    while True:
        ...
        phonenumber = driver.find_element(By.XPATH,'//*[@id="settings"]/div[3]/div[2]/div/div/form/div[2]/div[4]/div/div/div/div[1]/span')
    ...
    print(fg.light_red + f"[ Nike ] [ {time.strftime('%H:%M:%S', time.localtime())} ] [ {email} ]" + fg.dark_grey + " • " + fg.green + "Successfully checked account!")
    driver.quit()
    ...
    embed = DiscordEmbed(title=f'Successfully checked nike account', description='', color='bc252c')
    ...
    if phonenumber_exist == False:
        embed.add_embed_field(name='Phonenumber', value=f'❌ ||None||', inline=False)
    else:
        embed.add_embed_field(name='Phonenumber', value=f'✅ ||{phonenumber}||', inline=False)
    ...
    embed.set_timestamp()
    if phonenumber_exist == False:
        embed.add_embed_field(name='Phonenumber', value=f'❌', inline=False)
    else:
        embed.add_embed_field(name='Phonenumber', value=f'✅', inline=False)
    if show_name == "true":
...
with open('Nike/account_checker.csv', newline='') as input_file:
    ...
    try:
        for row in csv_file:
            if row != ["email", "password", "proxy", "countrycode"]:
                ...
                task += 1
                thread = threading.Thread(target=check_account, args=(row[0], row[1], row[2], row[3], ))
...
embed = DiscordEmbed(title=f'KenkiTools Nike Logger', description=f'{key_username} checking accounts', color="bc252c")
embed.set_footer(text=f"KenkiTools", icon_url="https://media.discordapp.net/attachments/878354931201998859/1059895159959076975/kenki_logo_render.png")
embed.set_timestamp()
