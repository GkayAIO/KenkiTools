import csv, threading, random, zipfile
...
def change_region(email, password, proxy, country_code, new_email):
    ...
    print(fg.light_red + f"[ Nike ] [ {time.strftime('%H:%M:%S', time.localtime())} ] [ {email} ]" + fg.dark_grey + " • " + fg.rs + "Changing account email...")
    ...
    while True:
        ...
        try:
            ...
            pass
        new_email_input = driver.find_element(By.XPATH,'//*[@id="email"]')
        time.sleep(1)
      
        for character in email:
       
            try:
                new_email_input.send_keys(Keys.BACK_SPACE)
        ...
        for character in new_email:
            new_email_input.send_keys(character)
            time.sleep(round(random.uniform(0.1, 0.35), 10))
        time.sleep(0.5)
        ...
        driver.find_element(By.XPATH,'//*[@id="settings"]/div[3]/div[2]/div/div/form/div[2]/div[7]/div[2]/div[2]').click()
        time.sleep(1)
        break
   
    print(fg.light_red + f"[ Nike ] [ {time.strftime('%H:%M:%S', time.localtime())} ] [ {email} ]" + fg.dark_grey + " • " + fg.green + "Successfully changed email of account!")
    driver.quit()
    ...
    embed = DiscordEmbed(title=f'Successfully changed account email', description='', color='bc252c')
    ...
    embed.add_embed_field(name='New Email', value=f'||{new_email}||', inline=True)
...
with open('Nike/email_changer.csv', newline='') as input_file:
    csv_file = csv.reader(input_file)
    try:
        for row in csv_file:
     
            if row != ["email", "new email", "password", "proxy", "countrycode"]:
                task += 1
                thread = threading.Thread(target=change_region, args=(row[0], row[2], row[3], row[4], row[1], ))
